...
import { ScheduleController, ScheduleSlotController, SlotController } from 'controller';
...
const initRoutes = ({ scheduleController, scheduleSlotController, slotController }: InitRoutesProps) => {
    const routes = Router();
    ...
    routes.get(
        SCHEDULE_MANAGER_BASE_ROUTE + ScheduleSlotManagerRoutes.GET_SCHEDULESLOT,
        wrap(scheduleSlotController.getById.bind(scheduleSlotController)),
    );

    routes.post(
        SCHEDULE_MANAGER_BASE_ROUTE + ScheduleSlotManagerRoutes.CREATE_SCHEDULESLOT,
        scheduleSlotController.create.bind(scheduleSlotController),
    );

    routes.delete(
        SCHEDULE_MANAGER_BASE_ROUTE + ScheduleSlotManagerRoutes.DELETE_SCHEDULESLOT,
        scheduleSlotController.delete.bind(scheduleSlotController),
    );

    routes.put(
        SCHEDULE_MANAGER_BASE_ROUTE + ScheduleSlotManagerRoutes.UPDATE_SCHEDULESLOT,
